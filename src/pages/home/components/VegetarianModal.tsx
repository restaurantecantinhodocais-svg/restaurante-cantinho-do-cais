...
const vegetarianDishes = [
  {
    ...
    image: "https://storage.readdy-site.link/project_files/9aa1dc26-bdac-4eb1-9493-30950be68091/152ea39c-f403-443c-9c8c-fc96b8085afe_IMG_20260606_170043.png?v=4cb8fb307ea660f0204e085af7ccadca",
  },
  {
    id: "salada-feijao",
    nameKey: "dish_salada_feijao",
    image: "https://storage.readdy-site.link/project_files/9aa1dc26-bdac-4eb1-9493-30950be68091/f84aa709-4edd-4d20-a5a8-67d0f39e6ba2_1780766690457.png?v=da76bb4d3b2606fa08ed8ef621cc8974",
  },
  {
    id: "salada-queijo-fresco",
    nameKey: "dish_salada_queijo_fresco",
    image: "https://storage.readdy-site.link/project_files/9aa1dc26-bdac-4eb1-9493-30950be68091/b704fa80-4116-47fa-b2b6-b0b6da778ff2_1780765740658.png?v=b8759b0d5637f0c929255c78183bd79f",
  },
];
...
export default function VegetarianModal({ isOpen, onClose, offset }: VegetarianModalProps) {
  ...
  useEffect(() => {
    const handleKey = (e: KeyboardEvent) => {
      ...
      if (e.key === "ArrowRight" && lightbox !== null) {
        setLightbox((prev) => (prev !== null ? (prev + 1) % vegetarianDishes.length : null));
      }
      if (e.key === "ArrowLeft" && lightbox !== null) {
        setLightbox((prev) => (prev !== null ? (prev - 1 + vegetarianDishes.length) % vegetarianDishes.length : null));
      }
    };
        ...
        >
          ...
          <div className="flex items-center justify-between px-6 py-5 border-b border-stone-100">
            <div>
              <span className="text-amber-700 text-xs font-semibold tracking-widest uppercase">
                {t("menu_cat_vegetarian")}
              </span>
              <h2 className="font-serif text-2xl text-stone-900 mt-0.5">{t("modal_vegetarian_title")}</h2>
            </div>
            <button
              onClick={onClose}
          ...
          <div className="overflow-y-auto p-6">
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
              {vegetarianDishes.map((item, index) => (
                <div
                  key={item.id}
      ...
      {lightbox !== null && (
        ...
        >
          ...
          <div className="flex flex-col items-center gap-4" onClick={(e) => e.stopPropagation()}>
            <img
              src={vegetarianDishes[lightbox].image}
              alt={t(vegetarianDishes[lightbox].nameKey)}
              className="max-h-[75vh] max-w-full rounded-xl object-contain"
            />
            <p className="font-serif text-white text-xl font-semibold">{lightbox + offset + 1} - {t(vegetarianDishes[lightbox].nameKey)}</p>
          </div>
          <button
          ...
          <div className="absolute bottom-4 left-1/2 -translate-x-1/2 text-white/60 text-sm">
            {lightbox + 1} / {vegetarianDishes.length}
          </div>
        </div>
      )}
    </>
