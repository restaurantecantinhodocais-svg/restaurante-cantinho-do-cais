...
export default function DessertsModal({ isOpen, onClose, offset }: DessertsModalProps) {
  ...
  useEffect(() => {
    const handleKey = (e: KeyboardEvent) => {
      ...
      if (e.key === "ArrowRight" && lightbox !== null) {
        setLightbox((prev) => (prev !== null ? (prev + 1) % desserts.length : null));
      }
      if (e.key === "ArrowLeft" && lightbox !== null) {
        setLightbox((prev) => (prev !== null ? (prev - 1 + desserts.length) % desserts.length : null));
      }
    };
        ...
        >
          ...
          <div className="flex items-center justify-between px-6 py-5 border-b border-stone-100">
            <div>
              ...
              <h2 className="font-serif text-2xl text-stone-900 mt-0.5">{t("modal_desserts_title")}</h2>
            </div>
            <button
              onClick={onClose}
      ...
      {lightbox !== null && (
        ...
        >
          ...
          <div className="flex flex-col items-center gap-4" onClick={(e) => e.stopPropagation()}>
            <img
              src={desserts[lightbox].image}
              alt={t(desserts[lightbox].nameKey)}
              className="max-h-[75vh] max-w-full rounded-xl object-contain"
            />
            <p className="font-serif text-white text-xl font-semibold">{lightbox + offset + 1} - {t(desserts[lightbox].nameKey)}</p>
          </div>
          <button
